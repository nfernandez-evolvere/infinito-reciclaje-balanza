...
### Tareas
- [ ] Vista `balanza`: 3 pasos secuenciales con Alpine.js
...
### Tests de integración
- `StorePesajeTest::test_operador_can_store_pesaje` — `POST /pesajes` con datos completos → HTTP 302, registro en DB
...
- `StorePesajeTest::test_admin_cannot_store_pesaje` — `POST /pesajes` como admin → HTTP 403
...
### Tareas
- [ ] Vista `historial`: KPIs del turno en el header (pesajes, toneladas netas, promedio, camiones en predio)
...
- `EgresoPesajeTest::test_operador_can_mark_egreso` — `POST /pesajes/{id}/egreso` → estado `Cerrado`, HTTP 302
- `EgresoPesajeTest::test_cannot_mark_egreso_on_closed_pesaje` — pesaje ya cerrado → HTTP 422
- `EditPesajeTest::test_operador_can_edit_own_pesaje` — `PUT /pesajes/{id}` con motivo → HTTP 302
...
- [ ] Ruta `POST /onboarding/visto` → `User::update(['onboarding_visto' => true])`; responde JSON `{ "ok": true }` para Alpine
...
### Tests de integración
- `OnboardingTest::test_modal_rendered_when_onboarding_not_seen` — `GET /balanza` con `onboarding_visto = false` → HTML contiene el componente modal
- `OnboardingTest::test_modal_not_rendered_when_already_seen` — `onboarding_visto = true` → componente no renderizado
- `OnboardingTest::test_post_visto_sets_flag` — `POST /onboarding/visto` → `onboarding_visto = true` en DB, respuesta JSON `{ "ok": true }`
